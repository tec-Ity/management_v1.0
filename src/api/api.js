...
let api_DNS = localStorage.getItem("DNS");
...
const fetchProm = (api, method, bodyObj) => {
  return new Promise(async (resolve) => {
    try {
      ...
      console.log(api_server, method, bodyObj);
      ...
      return resolve({
        status: 500,
        message: `[front] fetchProm Error ${error}`,
      });
    }
  });
};
...
export const axiosProm = async (api, method, formData) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      console.log(e);
      return resolve({ status: 600, message: "[front error] axiosProm" + e });
    }
  });
};
export const axios_Prom = async (api, method = "GET", formData, DNS) => {
  return new Promise(async (resolve, reject) => {
    try {
      // console.log("axios_Prom");
      if (DNS) api_DNS = DNS;
      if (!api) return resolve({ status: 400, message: "api 不能为空" });
      method = method.toUpperCase();
